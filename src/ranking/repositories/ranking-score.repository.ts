...
export class RankingScoreRepository {
    ...
    async updateRankingScore(rankingScoreId: string, data: Prisma.RankingItemScoreUncheckedUpdateInput) {
        try {
    ...
    async getAvgRankingItemScore(rankingItemId: string) {
        try {
            return (await this.prismaService.rankingItemScore.aggregate({
                where: {
                    rankingItemId
                },
                _avg: {
