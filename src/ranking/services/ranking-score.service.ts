...
export class RankingScoreService {
    ...
    async createRankingScore(createRankingScoreDto: CreateRankingItemScoreDto) {
        try {
            ...
            await Promise.all([
                 ...
                 this.rankingValidationsService.existRankingItemScore(rankingItemId, userId),
            ]);
            ...
            throw new BadRequestException('Não foi possível criar a pontuação do item do ranking 😔');
        }
    }
    ...
    async getRankingItemScores(rankingItemId: string) {
        try {
            ...
            throw new BadRequestException('Erro ao buscar as pontuações do ranking 😔');
        }
    }

    async updateRankingScore(rankingScoreId: string, score: number) {
        try {
            const rankingScore = await this.rankingScoreRepository.getRankingScoreById(rankingScoreId);

            if (!rankingScore) {
                throw new BadRequestException('Pontuação do item do ranking não encontrada 😔');
            }

            return await this.rankingScoreRepository.updateRankingScore(rankingScore.id, {
                score,
            });
        } catch (error) {
            if (error instanceof BadRequestException) {
            ...
            throw new BadRequestException('Não foi possível atualizar a pontuação do item do ranking 😔');
        }
    }
}
