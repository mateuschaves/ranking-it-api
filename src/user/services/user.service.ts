...
export class UserService {
    ...
    async createAccount(createAccountRequest: SignUpDto) {
        try {
            ...
            Logger.log('checking if email already exists', 'UserService.createAccount');
            const emailExist = await this.userRepository.findOne({ email: createAccountRequest.email });
            if (emailExist) {
                throw new BadRequestException('Email já cadastrado 🕵️‍♂️');
            }

            const passwordEncrypted = await this.encryptService.hash(createAccountRequest.password);
            ...
            const user = await this.userRepository.create({
                ...
                avatar: createAccountRequest.avatar,
            });
            ...
            throw new InternalServerErrorException('Ops! Não foi possível criar a conta 🥲');
        }
    }
    ...
    async login(email: string, password: string) {
        try {
            ...
            const passwordMatch = await this.encryptService.compare(user.password, password);
            if (!passwordMatch) {
            ...
            throw new InternalServerErrorException('Ops! Não foi possível fazer login 🥲');
        }
    }
}
